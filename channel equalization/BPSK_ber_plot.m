...
%hold on
semilogy(SNR_db,BER_wiener,'-+','LineWidth',1.5);
ylabel('Bit error rate')
xlabel('SNR(dB)')
title('Error performance using different equalization scheme')
grid on
hold on
semilogy(SNR_db,BER_ZF,'-o');
...
legend('Wiener filter','Zero Forcing','LMS,stepsize=0.075','LMS,stepsize=0.025','LMS,stepsize=0.0075');
